...
def convertFiles(logFile, inPath):
    ...
    for index, fileInfo in enumerate(aviFiles, 1):
        ...
        logMsg(logFile, f'    Start: {startTime}  ||  AVI Size: {fileInfo["size"]}  ||  {inputPath.replace("\\", "/")}')
        ...
        if result.returncode == 0 and os.path.exists(outputPath):
            ...
            try:
                if DELETE_AFTER:
                    os.remove(inputPath)
                    logMsg(logFile, f'    Deleted original AVI: {indexStr}  ||  {inputPath.replace("\\", "/")}')
                elif MOVE_FILE_PATH:
                    dest = os.path.abspath(MOVE_FILE_PATH)
                    os.makedirs(dest, exist_ok=True)
                    shutil.move(inputPath, os.path.join(dest, os.path.basename(inputPath)))
                    logMsg(logFile, f'    Moved original AVI to: {indexStr}  ||  {dest.replace("\\", "/")}')
                else:
                    logMsg(logFile, f'    Kept original AVI: {indexStr}  ||  {inputPath.replace("\\", "/")}')
