...
def searchFile(logFile, filePath, subStr):
    try:
            ...
            for target in subStr:
                if target in line:
                    ...
                    for j in range(start, end):
                        prefix = '>> ' if j == i else '   '
                        logMsg(logFile, f'{prefix}Line {j+1:04}: {lines[j]}', True)

                    logMsg(logFile, '\n' + ('=' * 80), True)
    except Exception as e:
        logMsg(logFile, f'[ERROR] Could not read file: {filePath.replace("\\", "/")}  ||  Reason: {str(e)}', True)


def scanPaths(logFile, searchPath, validExtensions, subStrings, recursiveSearch=True):
    for basePath in searchPath:
        if not os.path.exists(basePath):
            logMsg(logFile, f'[WARNING] Path does not exist: {basePath}', True)
            continue
        ...
        for root, dirs, files in os.walk(basePath):
            for file in files:
                if any(file.lower().endswith(ext) for ext in validExtensions):
                    ...
                    searchFile(logFile, fullPath, subStrings)
...
if __name__ == '__main__':
    ...
    try:
        ...
        scanPaths(logFile, SEARCH_PATHS, VALID_EXTENSIONS, SUBSTRINGS, RECURSIVE_SEARCH)
